...
function filtrowanie($find) {
    $tekst = '';
    if($find != null) foreach ($find as $klucz => $wartosc) $tekst .= 'find['.$klucz.']=' . $wartosc . '&'; 
    return $tekst;
}
...
?>
    <table>
        <tr>
            <th>
                <a href="Tabelka2.php?<?= filtrowanie(isset($_GET['find']) ? $_GET['find'] : null) ?>sort=id&amp;dir=<?= (isset($_GET['dir']) && $_GET['dir'] == 'desc') ? 'asc' : 'desc' ?>">
                    ID
                </a>
            </th>
            <th>
                <a href="Tabelka2.php?<?= filtrowanie(isset($_GET['find']) ? $_GET['find'] : null) ?>sort=prod_name&amp;dir=<?= (isset($_GET['dir']) && $_GET['dir'] == 'desc') ? 'asc' : 'desc' ?>">
                    Nazwa produktu
                </a>
            </th>
            <th>
                <a href="Tabelka2.php?<?= filtrowanie(isset($_GET['find']) ? $_GET['find'] : null) ?>sort=price&amp;dir=<?= (isset($_GET['dir']) && $_GET['dir'] == 'desc') ? 'asc' : 'desc' ?>">
                    Cena
                </a>
            </th>
            <th>
                <a href="Tabelka2.php?<?= filtrowanie(isset($_GET['find']) ? $_GET['find'] : null) ?>sort=quantity&amp;dir=<?= (isset($_GET['dir']) && $_GET['dir'] == 'desc') ? 'asc' : 'desc' ?>">
                    Ilość
                </a>
            </th>
            <th>
...
?>
        <tr>
            ...
            <td><?= $row['quantity'] ?></td>
            <td>
                <a id="update" href="update.php?id=<?=$row['id'] ?>">Edytuj</a> |
                <a id="remove" href="Tabelka2.php?action=remove&id=<?=$row['id'] ?>">Usun</a>
            </td>
        </tr>
<?php
    }
